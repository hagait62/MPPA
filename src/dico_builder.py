...
def build_multi_pairwise_dictionary(embs, params, s2t_candidates=None, t2s_candidates=None):

    dico = defaultdict(dict)
    for src, tgt in itertools.permutations(params.langs, 2):
        dico[src][tgt] = build_pairwise_dictionary(
            embs[src],embs[tgt],params, s2t_candidates, t2s_candidates, return_tensor=True)
    return dico
